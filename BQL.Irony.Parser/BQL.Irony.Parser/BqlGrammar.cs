...
namespace bql
{
    [Language("BQL", "0.0.1", "BQL Grammar")]
    public class BqlGrammar : Grammar
    {
        public BqlGrammar()
            : base(false /* SQL is case insensitive */ )
        {
            ...
            // LMR
            var PROJECTION = ToTerm("PROJECTION");
            ...
            var outerJoinOpt = new NonTerminal("outerJoinOpt");
            ...
            // LMR
            var projectionDecl = new NonTerminal("projectionDecl");
            var projectionRef = new NonTerminal("projectionRef");
            var selectClause = new NonTerminal("selectClause");
            var queryExcludingSelect = new NonTerminal("queryExcludingSelect");
            ...
            stmtList.Rule
                = MakePlusRule(stmtList, stmtLine);

            // ID
            Id.Rule
                = MakePlusRule(Id, dot, Id_simple);
            ...
            // Create table
            createTableStmt.Rule
                = CREATE + TABLE + Id + "(" + fieldDefList + ")" + constraintListOpt;

            fieldDefList.Rule
                = MakePlusRule(fieldDefList, comma, fieldDef);

            fieldDef.Rule
                = Id + typeName + typeParamsOpt + nullSpecOpt;
            ...
            constraintDef.Rule
                = CONSTRAINT + Id + constraintTypeOpt;

            constraintListOpt.Rule
                = MakeStarRule(constraintListOpt, constraintDef);
            ...
            idlist.Rule
                = MakePlusRule(idlist, comma, Id);

            // Create Index
            createIndexStmt.Rule
                = CREATE + uniqueOpt + INDEX + Id + ON + Id + orderList + withClauseOpt;
            ...
            orderList.Rule
                = MakePlusRule(orderList, comma, orderMember);
            ...
            // Insert stmt
            insertStmt.Rule
                = INSERT + intoOpt + Id + idlistPar + insertData;
            ...
            intoOpt.Rule
                = Empty
                | INTO; //Into is optional in MSSQL

            // Update stmt
            updateStmt.Rule
                = UPDATE + Id + SET + assignList + whereClauseOpt;

            assignList.Rule
                = MakePlusRule(assignList, comma, assignment);
            ...
            // Delete stmt
            deleteStmt.Rule
                = DELETE + FROM + Id + whereClauseOpt;

            // Select stmt
            selectStmt.Rule
                = selectClause + fromClauseOpt + whereClauseOpt + groupClauseOpt + havingClauseOpt + orderClauseOpt
                | fromClause + whereClauseOpt + groupClauseOpt + havingClauseOpt + orderClauseOpt + selectClause
                ;


            selectClause.Rule
                = SELECT + selRestrOpt + selList + intoClauseOpt;
            ...
            columnItemList.Rule
                = MakePlusRule(columnItemList, comma, columnItem);

            columnItem.Rule
                = projectionRef
                | columnSource + aliasOpt;
            ...
            joinChainOpt.Rule
                = Empty
                //| joinKindOpt + JOIN + Id + aliasOpt
                | joinKindOpt + JOIN + idlist + ON + Id + "=" + Id;
            //| joinKindOpt + JOIN + Id + aliasOpt + ON + Id
            //| joinKindOpt + JOIN + Id + aliasOpt + WITH + Id;
            ...
            // projections
            projectionDecl.Rule
                = PROJECTION + Id_simple + "{" + selList + "}";

            projectionRef.Rule
                = ToTerm("<") + Id_simple + ">"
                | Id_simple + dot + "<" + Id_simple + ">";


            // Expression
            exprList.Rule
                = MakePlusRule(exprList, comma, expression);
            ...
            betweenExpr.Rule
                = expression + notOpt + "BETWEEN" + expression + "AND" + expression;
